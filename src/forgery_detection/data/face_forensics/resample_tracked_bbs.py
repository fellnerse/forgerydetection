...
def resample_tracked_bbs(resampled_data_dir_root, bb_data_dir_root, compressions):
    ...
    for resampled_videos, tracked_bbs, resampled_face_images in zip(
        ...
        for video in tqdm(sorted(resampled_videos.iterdir())):
            ...
            relative_bb = tracked_bbs / video.with_suffix("").name / "relative_bb.json"
            resampled_tracked_bb = (
                resampled_face_images
                / video.with_suffix("").name
                / "tracked_bb_new.json"
            )
            resampled_tracked_bb.parent.mkdir(parents=True, exist_ok=True)
            resampled_relative_bb = (
                resampled_face_images / video.with_suffix("").name / "relative_bb.json"
            )

            with open(tracked_bb, "r") as f:
                tracked_bb_dict = json.load(f)
            with open(relative_bb, "r") as f:
                relative_bb_dict = json.load(f)
            ...
            if tracked_bb_length == resampled_video_length:
                ...
                shutil.copy(str(relative_bb), str(resampled_relative_bb))
            else:
                ...
                relative_bb_values = list(relative_bb_dict.values())
                ...
                resampled_bbs = {}
                # todo do same for relative stuff as well
                resampled_relative_bbs = {}
                for _i, idx in enumerate(resampled_idx):
                    ...
                    next_relative_val = relative_bb_values[idx]
                    if (
                        idx != 0
                        ...
                        resampled_relative_bbs[f"{_i:04d}"] = None
                    else:
                        resampled_bbs[f"{_i:04d}"] = next_val
                        resampled_relative_bbs[f"{_i:04d}"] = next_relative_val

                # with open(resampled_tracked_bb, "w") as f:
                #     json.dump(resampled_bbs, f)

                with open(resampled_relative_bb, "w") as f:
                    json.dump(resampled_relative_bbs, f)
