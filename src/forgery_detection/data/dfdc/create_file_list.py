...
train_data_numbers = [1, 2, 3, 4]
val_data_numbers = [0]

for train_data_number in train_data_numbers:
    for label in (root_dir / f"extracted_images_{train_data_number}").iterdir():
...
for val_data_number in val_data_numbers:
    for label in (root_dir / f"extracted_images_{val_data_number}").iterdir():
        images = list(label.glob("*/*.png"))
...
f.save("/data/ssd1/file_lists/dfdc/0_2_split.json")

for split in [TRAIN_NAME, VAL_NAME, TEST_NAME]:
    data_set = FileList.get_dataset_form_file(
        "/data/ssd1/file_lists/dfdc/0_2_split.json", split
    )
