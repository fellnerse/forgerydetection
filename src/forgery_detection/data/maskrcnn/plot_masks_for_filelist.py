...
d = f.get_dataset("train", tensor_transforms=[unnormalize])

m = maskrcnn_resnet50_fpn(pretrained=True).eval()
...
#%%
im, label = d[22551 + 8, 22551 + 8]
result = m(im.unsqueeze(0))
for mask, label, score in zip(
    result[0]["masks"], result[0]["labels"], result[0]["scores"]
):
    rgb_mask = (im * mask.expand(3, -1, -1)).permute((1, 2, 0))
    plt.imshow(rgb_mask.detach().numpy())
    plt.title(f"{s[label]}: {score}")
    plt.show()
